  ...
  if (secondary_raked) {
    ...
    if (input$data_collection_method == "Attempted Census") {
      ...
      survey_design <- svydesign(
        ...
        ids = as_survey_formula(input$psu_cluster_variables, ids = T),
        nest = TRUE,
        fpc = as_survey_formula(input$population_size_variables)
      )      
    }
...
render_analysis_output <- function(output_table) {
  ...
  renderUI({
    if("error" %in% class(output_table)) {
      ...
      for(i in 1:ncol(output_table)){
        ...
        if (names(output_table)[i] %in% routine_dictionary$column_name){
          ...
          precision_to_use <- routine_dictionary$precision[which(names(output_table)[i] == routine_dictionary$column_name[routine_dictionary$routine_name == routine_name])]
          if (is.double(output_table[,i])) {
            ...
            digit <- precision_to_use
            output_table[,i] <- output_table[,i] %>% mutate(across(where(is.numeric), round, digit))
          }
          # checks if output column is labeled as percent
          if (routine_dictionary$is_percentage[which(names(output_table)[i] == routine_dictionary$column_name)] == TRUE) {
...
observeEvent(input$input_file, {
  ...
  for(i in 1:nrow(input$input_file)) {
    ...
    if(grepl("\\.csv$", base_filename)) {
      new_data <- readr::read_csv(full_filename)
    } else if(grepl("\\.xlsx$", base_filename)) {
      new_data <- openxlsx::read.xlsx(full_filename)
    } else if(grepl("\\.sas7bdat$", base_filename)) {
      new_data <- haven::read_sas(full_filename)
    } else if(grepl("\\.sav$", base_filename)) {
      new_data <- haven::read_sav(full_filename)
    }
...
observeEvent({input$output_files_df_cell_clicked; input$input_file}, {
  # if((length(input$output_files_df_cell_clicked) == 0) & !(is.null(str(input$output_files_df_cell_clicked)))) {
  #   cat(str(input$output_files_df_cell_clicked))
  #   clicked_row <- nrow(global_values$file_metadata)
  # } else if(!(is.null(input$output_files_df_cell_clicked))){
...
h5("For each category of response and eligibility status, select the appropriate value:", style = 
...
disabled(selectizeInput(
    ...
    label = "Select the variables, if any, giving the population size. Use one variable for each stage of sampling.",
    multiple = TRUE,
    choices = NULL,
    width = "95%"
))
...
div(
  ...
  disabled(selectizeInput(
    ...
    label = "Select the variable in your dataset that indicates the sampling weight for each case.",
    choices = NULL,
    multiple = FALSE,
    width = "95%"
  )),
...
observeEvent(input$t4_analysis_compare_sample_submit, {
  ...
  output_table <- tryCatch({
    ...
    if (input$t4_t_full_or_elig == "Respondents vs. All sampled cases (regardless of response or eligibility status)") {
      param_list <<- list(
      "survey_design" = survey_design,
...
observeEvent(input$t5_analysis_adjust_weights_submit, {
  ...
  output_table <- tryCatch({
    ...
    if (input$t5_replicate_weight_method == "Bootstrap (Recommended)") {
      ...
      orig_rep_design <- as.svrepdesign(
        design = global_values$survey_design,
        type = ifelse(isTRUE(global_values$survey_design$has.strata), "JKn", "JK1")
      )
    }
    ...
    tagList(
      ...
      hidden(tags$div(
        ...
        numericInput(
          ...
          label = "Maximum number of iterations for stewise algorithm",
          value = 100,
          step = 1
        ) 
      )),
...
observeEvent(input$analysis_predict_response_submit, {
  ...
  output_table <- tryCatch({
    ...
    routine_label <<- routines[which(routines$routine_name == routine_name), "routine_label"]
    do.call(predict_response_status_via_glm, param_list)
...
observeEvent(input$analysis_aux_predictive_submit, {
  ...
  output_table <- tryCatch({
    ...
    routine_label <<- routines[which(routines$routine_name == routine_name), "routine_label"]
    do.call(predict_outcome_via_glm, param_list)
...
observeEvent(input$analysis_across_subgroups_submit, {
  ...
  htest_obj <- survey::svychisq(design = subsetted_survey_design, 
            ...
            input[[paste0("cb_", (report_index + (previous_report_input_counter - 1)))]] == TRUE)
        {
          ...
          for(z in 1:ncol(output_table)){
            ...
            if (names(output_table)[z] %in% routine_dictionary$column_name) {
              if (routine_dictionary$is_percentage[which(names(output_table)[z] == routine_dictionary$column_name)] == TRUE){
                numFmt <- "0."
          ...
          writeData(excel_book, sheet_name, x = output_table, startRow = 3, withFilter = TRUE)
...
Reference Guide {data-navmenu="Resources"}
=====================================


<external_link data-href="https://ideadata.org/">IDEA Data Center</external_link> {data-navmenu="Resources"}
=====================================
